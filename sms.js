(function($) {
	$.fn.objSms = function(InIt) {
		...
		function SetLocation() {
			...
			if("mDeviceInbox" == menuId){
				$('#lt_sms_btnImport').show();
			}else{
				$('#lt_sms_btnImport').hide();
			}
		...
		function AddSmsToList(smsId, address, contactId, date, read, location, status, body, protocol, type) {
			...
			var htmlText = "<tr><td id=\"" + contactId + "\" name=\"" + address + "\"" + tdStyle + ">" + contactName + "</td>" + "<td style=\"cursor: pointer;\" id=\"" + smsId + "\"><span class=\"row\">" + body + "</span></td>" + "<td><span>" + date + "</span></td>" + "<td> <img src=\"" + imgSrc + "\"/></td>" + "<td><input type=\"checkbox\"/></td></tr>";

			$("#smsListInfo").append(htmlText);
			var htmlTextPhone = "<tr><td id=\"" + contactId + "\" name=\"" + address + "\"" + tdStyle + ">" + contactName +	"</td>" + "<td style=\"cursor: pointer;\" id=\"" + smsId + "\" ><div class=\"row\">" + body + "<div></td>" + "<td> <img src=\"" + imgSrc + "\"/></td>" + "<td><input type=\"checkbox\"/></td></tr>";
		...
		function BindEventForSmsList() {
			$('#lt_sms_btnImport').click(function(){
				GetSimStatus();
				if (!bSimPresent) {
					showMsgBox(jQuery.i18n.prop("lt_sms_btnImport"), jQuery.i18n.prop("lsmsSimCardAbsent"));
					return;
				}
				$('#lt_confirmDlg_title').html(jQuery.i18n.prop("mDeviceInbox"));
				$('#lt_confirmDlg_msg').html(jQuery.i18n.prop("lImportSMSCheck"));
				if(g_bodyWidth>430){
					ShowDlg("confirmDlg",380,154);
				}else{
				...
				$('#lt_btnConfirmYes').click(function(){
					...
					setTimeout(function(){
						var retXml = PostXml(gSmsModelName, "sms.copy_sim_to_local");
						var count = $(retXml).find('count').text();
						UpdateSmsList(currentPage,true);
						showMsgBox(jQuery.i18n.prop("lt_sms_btnImport"), jQuery.i18n.prop("SIMImportSucc")+" "+count);
					},200);
			...
			$("tbody tr").each(function() {
				...
				$(this).children("td:eq(1)").unbind("click").click(function() {
					...
					var formatTime = "20" + date[0] + "/" + date[1] + "/" + date[2] + " " + date[3] + ":" + date[4] + ":" + date[
						5];
		...
		function SavePhoneBook(phoneNumber) {
			...
			$("#lt_Phonebook_btnSave").click(function() {
				...
				if (IsChineseChar($("#txtName").val()) && $("#txtName").val().length > 6) {
					$("#lt_Phonebook_stcInputCheckout").show().text(jQuery.i18n.prop("lt_Phonebook_stcName") + ":" + jQuery.i18n.prop(
				...
				if (0 != $(retXml).find("result").text()) {
					...
					currentPage = $("#divSmsPageNum .pageSelIdx").text();
					UpdateSmsList($("#divSmsPageNum .pageSelIdx").text(), false); //refresh sms list to show contact name.
					showMsgBoxAutoClose(jQuery.i18n.prop("dialog_message_sim_title"), jQuery.i18n.prop(
						"dialog_message_sim_save_contact_success"));
				}
		...
		function DeleteSms() {
			...
			if (0 == $(retXml).find("resp").text()) {
				...
				UpdateSmsList(1, true);
				showMsgBoxAutoClose(jQuery.i18n.prop("dialog_message_sim_title"), jQuery.i18n.prop(
					"dialog_message_sim_delete_sms_success"));
			} else {
